...
/**
 * The class chatUsersList implements a  list of users which is stored in a 
 * HashMap type of Map keeping a key-value pair as reference of the stored data.
 * A data representation would look like this: MyList[11,'ADOLFO'].
 * The class is Serializable so an instance of it can be sent over stream channels
 * to ease reading/writing operations. Most of its methods are synchornized to 
 * ensure that only one Thread instance is working on the list.
 * 
